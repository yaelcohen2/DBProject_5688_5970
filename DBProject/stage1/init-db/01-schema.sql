...
COMMENT ON TABLE HOUSEKEPINGTASK IS 'Scheduled cleaning assignments assigned to specific rooms';
...
CREATE TABLE CLEANNINGLOG (
  ...
  CONSTRAINT fk_log_task FOREIGN KEY (taskID) REFERENCES HOUSEKEPINGTASK(taskID),
  CONSTRAINT fk_log_employee FOREIGN KEY (employeeID) REFERENCES HOUSEKEEPINGEMPLOYEE(employeeID)
);

COMMENT ON TABLE CLEANNINGLOG IS 'Real-time tracking of when cleaning started and finished for a task';
...
CREATE TABLE CLEANNINGSUPPLIES (
  suppliesID SERIAL PRIMARY KEY,
  name VARCHAR(100) NOT NULL,
  quantity INT NOT NULL DEFAULT 0 -- Current stock level
);

COMMENT ON TABLE CLEANNINGSUPPLIES IS 'Inventory of cleaning materials, chemicals, and tools';
...
CREATE TABLE BELONGSTO (
  ...
  CONSTRAINT fk_belongs_task FOREIGN KEY (taskID) REFERENCES HOUSEKEPINGTASK(taskID)
);
...
CREATE TABLE USES (
  ...
  CONSTRAINT fk_uses_supply FOREIGN KEY (suppliesID) REFERENCES CLEANNINGSUPPLIES(suppliesID),
  CONSTRAINT fk_uses_task FOREIGN KEY (taskID) REFERENCES HOUSEKEPINGTASK(taskID)
);
...
COMMENT ON COLUMN HOUSEKEPINGTASK.priority IS 'Priority level: 1 (Low) to 5 (Urgent)';
